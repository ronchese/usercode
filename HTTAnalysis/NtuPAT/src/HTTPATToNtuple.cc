...
HTTPATToNtuple::HTTPATToNtuple( const edm::ParameterSet& ps ) {
...
HTTPATToNtuple::~HTTPATToNtuple() {
}


void HTTPATToNtuple::beginJob() {
//  cout << "HTTPATToNtuple::beginJob()" << endl;
  TDirectory* current = gDirectory;
  gROOT->cd();
  HTTAnalyzer::beginJob();
  openNtuple( ntuName );
  gROOT->cd();
  book();
  current->cd();
...
void HTTPATToNtuple::openNtuple( const std::string& name ) {
  TDirectory* current = gDirectory;
...
void HTTPATToNtuple::read( const edm::EventBase& ev ) {
...
void HTTPATToNtuple::closeNtuple() {
  close();
  file->Close();
  return;
}
...
void HTTPATToNtuple::fillTrigger() {
  return;
}


void HTTPATToNtuple::fillMet() {
...
void HTTPATToNtuple::fillMuons() {
  ...
  muoIso         ->resize( nObj, 0.0 );
  muoClass       ->resize( nObj, 0.0 );
...
void HTTPATToNtuple::fillElectrons() {
...
void HTTPATToNtuple::fillGenParticles() {
